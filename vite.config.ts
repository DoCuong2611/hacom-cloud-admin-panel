...
export default defineConfig(({ mode }) => {
  ...
  const cloudProxyTarget = resolveHttpProxyTarget(
    env.VITE_DEV_CLOUD_PROXY_TARGET ?? env.VITE_DEV_ADMIN_PROXY_TARGET,
    'http://localhost:8080',
  );
    ...
    server: {
      ...
      proxy: {
        '/api/v1/admin': {
          ...
          changeOrigin: true,
          secure: false,
          configure: rewriteOriginToTarget(cloudProxyTarget),
        },
        '/api/v1/auth': {
